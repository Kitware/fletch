...
if(fletch_ENABLE_CPython)
  ...
  set_property(CACHE CPython_SELECT_VERSION PROPERTY STRINGS "3.6.15" "3.8.13" "3.10.4" "3.12.12" "3.14.2" "3.14.2t")
  set(fletch_PYTHON_MAJOR_VERSION 3 CACHE INTERNAL "Required for CPython" FORCE)

  # Check for free-threaded (GIL-less) build suffix 't'
  set(CPython_free_threaded OFF)
  string(REGEX MATCH "t$" _has_t_suffix "${CPython_SELECT_VERSION}")
  if(_has_t_suffix)
    set(CPython_free_threaded ON)
    string(REGEX REPLACE "t$" "" _version_no_suffix "${CPython_SELECT_VERSION}")
  else()
    set(_version_no_suffix "${CPython_SELECT_VERSION}")
  endif()

  string(REPLACE "." ";" CPython_VERSION_LIST ${_version_no_suffix})
  ...
  if(_version_no_suffix VERSION_EQUAL 3.6.15)
  ...
  elseif(_version_no_suffix VERSION_EQUAL 3.8.13)
  ...
  elseif(_version_no_suffix VERSION_EQUAL 3.10.4)
  ...
  elseif(_version_no_suffix VERSION_EQUAL 3.12.12)
    set(CPython_url "https://data.kitware.com/api/v1/file/697146efeddae180bf72f051/download/cpython.${CPython_full_version}.zip")
    set(CPython_md5 "b0de544602016399b62bf3d35cdcb385")
  elseif(_version_no_suffix VERSION_EQUAL 3.14.2)
    set(CPython_url "https://data.kitware.com/api/v1/file/697146fbeddae180bf72f054/download/cpython.${CPython_full_version}.zip")
    set(CPython_md5 "728c3bd9630a7c3088fa09930b052fc8")
  else()
...
#OpenBLAS
if(NOT WIN32)
  set(OpenBLAS_SELECT_VERSION 0.3.30 CACHE STRING "Select the version of OpenBLAS to build.")
  set_property(CACHE OpenBLAS_SELECT_VERSION PROPERTY STRINGS "0.3.30" "0.3.21" "0.3.10" "0.3.6")
  ...
  elseif (OpenBLAS_version VERSION_EQUAL 0.3.30)
    set(OpenBLAS_url "https://data.kitware.com/api/v1/file/6936e92d687bfaed77250c41/download/openblas.${OpenBLAS_version}.tar.gz")
    set(OpenBLAS_md5 "8db3d57f4d4485c6ae3f21ea465660e7")
  else()
...
if (fletch_ENABLE_PostgreSQL OR fletch_ENABLE_ALL_PACKAGES)
  ...
  elseif(PostgreSQL_SELECT_VERSION VERSION_EQUAL 10.23)
    ...
    if(WIN32)
      set(PostgreSQL_binaries_url "https://get.enterprisedb.com/postgresql/postgresql-${PostgreSQL_version}-1-windows-x64-binaries.zip")
      set(PostgreSQL_binaries_md5 "5e01e77deb12cd7f0c4df24903694098")
    endif()
  else()
...
if (fletch_ENABLE_pybind11 OR fletch_ENABLE_ALL_PACKAGES)
  set(pybind11_SELECT_VERSION 2.13.6 CACHE STRING "Select the version of pybind11 to build.")
  set_property(CACHE pybind11_SELECT_VERSION PROPERTY STRINGS "2.5.0" "2.8.1" "2.10.3" "2.13.6")
endif()
...
elseif (pybind11_version VERSION_EQUAL 2.8.1)
  set(pybind11_url "https://github.com/pybind/pybind11/archive/v${pybind11_version}.tar.gz")
  set(pybind11_md5 "06d173fb97f1d2d804d318e47f924892" )
...
elseif (pybind11_version VERSION_EQUAL 2.13.6)
  set(pybind11_url "https://github.com/pybind/pybind11/archive/refs/tags/v${pybind11_version}.tar.gz")
  set(pybind11_md5 "a04dead9c83edae6d84e2e343da7feeb" )
...
if(fletch_ENABLE_qtExtensions OR fletch_ENABLE_ALL_PACKAGES)
  ...
  elseif(qtExtensions_SELECT_VERSION STREQUAL "20200330")
    ...
    set(qtExtensions_url "https://data.kitware.com/api/v1/file/682fc03c0dcd2dfb445a5433/download/qtextensions.${qtExtensions_version}.tar.gz")
