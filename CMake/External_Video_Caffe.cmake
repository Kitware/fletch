if (WIN32)
  message( FATAL_ERROR "Video-Caffe is not yet supported on windows" )
  return()
endif()

if (NOT fletch_BUILD_WITH_CUDA)
  message( FATAL_ERROR "Video-Caffe requires CUDA" )
  return()
endif()
if (NOT fletch_BUILD_WITH_CUDNN)
  message( FATAL_ERROR "Video-Caffe requires CUDNN" )
  return()
endif()
...
option(AUTO_ENABLE_VIDEO_CAFFE_DEPENDENCY "Automatically turn on all video-caffe dependencies if video-caffe is enabled" OFF)
if(fletch_ENABLE_Video_Caffe AND AUTO_ENABLE_VIDEO_CAFFE_DEPENDENCY)
...
function(addVideoCaffeDendency depend version)
  ...
  else() #need to make sure library is built before video-caffe
    set(Video_Caffe_DEPENDS ${Video_Caffe_DEPENDS} ${depend} PARENT_SCOPE)
  endif()
...
if(NOT WIN32) # Win32 build takes care of most dependencies automatically
  addVideoCaffeDendency(LevelDB "")
  addVideoCaffeDendency(LMDB "")
  if(NOT APPLE)
    addVideoCaffeDendency(OpenBLAS "")
  endif()
  addVideoCaffeDendency(Protobuf "")
endif()
addVideoCaffeDendency(HDF5 "") # VideoCaffe for windows grabs its own HDF5, but we need a parallel builds so we don't break other code
addVideoCaffeDendency(Boost 1.46)
addVideoCaffeDendency(GFlags "")
addVideoCaffeDendency(GLog "")
addVideoCaffeDendency(OpenCV "")
addVideoCaffeDendency(ZLib "")

if(NOT allOk)
  message(FATAL_ERROR "Missing dependency(ies).")
endif()

# Set paths which VideoCaffe requires for protobuf and opencv

set( VIDEO_CAFFE_PROTOBUF_ARGS )
...
if(fletch_ENABLE_LevelDB)
  ...
  # NOTE: VideoCaffe currently has LevelDB_INCLUDE instead of the normal LevelDB_INCLUDE_DIR
  set( VIDEO_CAFFE_LevelDB_ARGS
...
if(fletch_ENABLE_GFlags)
  ...
  set( VIDEO_CAFFE_GFlags_ARGS -DGFLAGS_ROOT_DIR:PATH=${GFlags_DIR})
endif()
...
if(fletch_ENABLE_OpenBLAS)
  ...
  set(VIDEO_CAFFE_OPENBLAS_ARGS "-DOpenBLAS_INCLUDE_DIR=${OpenBLAS_ROOT}/include"
    "-DOpenBLAS_LIB=${OpenBLAS_ROOT}/lib/${openblas_libname}")
else()
  set(VIDEO_CAFFE_OPENBLAS_ARGS "-DOpenBLAS_INCLUDE_DIR=${OpenBLAS_INCLUDE_DIR}"
    "-DOpenBLAS_LIB=${OpenBLAS_LIBRARY}")
endif()

if(fletch_BUILD_WITH_CUDA)
  format_passdowns("CUDA" CUDA_BUILD_FLAGS)
  set( VIDEO_CAFFE_GPU_ARGS
    ${CUDA_BUILD_FLAGS}
    -DCPU_ONLY:BOOL=OFF
    )
...
set (Video_Caffe_PATCH_DIR "${fletch_SOURCE_DIR}/Patches/Video_Caffe/${Video_Caffe_version}")
if (EXISTS ${Video_Caffe_PATCH_DIR})
  set(
    Video_Caffe_PATCH_COMMAND ${CMAKE_COMMAND}
    -DVideo_Caffe_patch=${Video_Caffe_PATCH_DIR}
    -DVideo_Caffe_source=${fletch_BUILD_PREFIX}/src/Video_Caffe
    -P ${Video_Caffe_PATCH_DIR}/Patch.cmake
    )
else()
  set(Video_Caffe_PATCH_COMMAND "")
endif()


# Main build and install command
if(WIN32)
ExternalProject_Add(Video_Caffe
  DEPENDS ${Video_Caffe_DEPENDS}
  URL ${Video_Caffe_url}
  URL_MD5 ${Video_Caffe_md5}
  PREFIX ${fletch_BUILD_PREFIX}
  DOWNLOAD_DIR ${fletch_DOWNLOAD_DIR}
  INSTALL_DIR ${fletch_BUILD_INSTALL_PREFIX}

  PATCH_COMMAND ${Video_Caffe_PATCH_COMMAND}
  ...
  CMAKE_ARGS
    ...
    -DCAFFE_FORK_SUFFIX:STRING=Video
    ${VIDEO_CAFFE_OPENCV_ARGS}
    ${PYTHON_ARGS}
    ${VIDEO_CAFFE_GPU_ARGS}
)
else()
ExternalProject_Add(Video_Caffe
  ...
  CMAKE_ARGS
    ...
    -DCAFFE_FORK_SUFFIX:STRING=Video
    ${PYTHON_ARGS}
    ${VIDEO_CAFFE_PROTOBUF_ARGS}
    ${VIDEO_CAFFE_OPENCV_ARGS}
    ${VIDEO_CAFFE_LMDB_ARGS}
    ${VIDEO_CAFFE_LevelDB_ARGS}
    ${VIDEO_CAFFE_GLog_ARGS}
    ${VIDEO_CAFFE_GFlags_ARGS}
    ${VIDEO_CAFFE_HDF5_ARGS}
    ${VIDEO_CAFFE_OPENBLAS_ARGS}
    ${VIDEO_CAFFE_GPU_ARGS}
  )
endif()

fletch_external_project_force_install(PACKAGE Video_Caffe)

set(Video_Caffe_ROOT ${fletch_BUILD_INSTALL_PREFIX} CACHE STRING "")

file(APPEND ${fletch_CONFIG_INPUT} "
########################################
# Video Caffe
########################################
set(Video_Caffe_ROOT    \${fletch_ROOT})
")
