                         ...
                         vidl_frame      & out_frame)
{
  ...
  unsigned out_size = (unsigned)av_image_get_buffer_size(out_fmt, ni, nj, 1);
#else
...
#if LIBAVCODEC_VERSION_MAJOR >= 59
  // FFmpeg 5.0+ API: use av_image_fill_arrays instead of avpicture_fill
  uint8_t* in_data[4];
  int in_linesize[4];
  av_image_fill_arrays(in_data, in_linesize, (uint8_t*)in_frame.data(), in_fmt, ni, nj, 1);

  uint8_t* out_data[4];
  int out_linesize[4];
  av_image_fill_arrays(out_data, out_linesize, (uint8_t*)out_frame.data(), out_fmt, ni, nj, 1);
#else
  AVPicture in_pic;
